...
class Config:
    ...
    plot_path: str = "q_learning_cartpole_vs_baseline_updated.png"
...
class QLearningAgent:
    def __init__(self, obs_dim: int, action_dim: int, cfg: Config, device="cpu"):
        ...
        if self.use_target_network:
            self.target_net = QNetwork(obs_dim, action_dim, cfg.hidden_sizes).to(self.device)
    ...
    def update(self, state, action, reward, next_state, done):
        if self.use_replay_buffer:
            ...
            next_state_t = torch.tensor(next_states, dtype=torch.float32, device=self.device)
            action_t = torch.tensor(actions, dtype=torch.long, device=self.device).unsqueeze(1)
            reward_t = torch.tensor(rewards, dtype=torch.float32, device=self.device).unsqueeze(1)
...
def plot_learning_curve(return_steps, returns, cfg: Config):
    ...
    # agent
    plt.plot(return_steps, returns, alpha=0.25, label="Q-learning (raw)")
    if len(returns) >= 20:
        smoothed = moving_average(returns, window=20)
        smoothed_steps = return_steps[19:]
        plt.plot(smoothed_steps, smoothed, linewidth=2, label="Q-learning (smoothed)")
    ...
    plt.ylabel("Return")
    plt.title("CartPole: Q-Learning vs Baseline")
    plt.legend()
